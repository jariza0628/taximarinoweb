...
@Component({
  selector: 'app-reporte-comisionistas-general',
  templateUrl: './reporte-comisionistas-general.component.html',
  styleUrls: ['./reporte-comisionistas-general.component.css']
})
export class ReporteComisionistasGeneralComponent implements OnInit {
  ...
  search() {
    ...
    if (this.date2 && this.date) {
      let subscription = this.GN.getSalesByDateAndComisionGeneral(
        "sales",
        "",
        this.date,
        this.date2,
      ).subscribe((data) => {
        console.log('datess', this.date + ' ' + this.date2);
      ...
      setTimeout(() => {
        ...
        this.ordenar();
        console.log('*************************************************');
        this.comisionistasTotales = [];
        let sumaComisionistas = {};

        this.data.forEach(venta => {
          let comisionista = venta.comisionista;
          let totalComision = venta.totalComison;
    
          // Buscar si el comisionista ya existe en el array
          let comisionistaExistente = this.comisionistasTotales.find(c => c.comisionista === comisionista);
    
          if (comisionistaExistente) {
            // Si el comisionista ya existe, agregar la comisión
            comisionistaExistente.valor += totalComision;
          } else {
            // Si no existe, agregar un nuevo objeto al array
            this.comisionistasTotales.push({ comisionista: comisionista, valor: totalComision });
          }
        });
